...
class RealmService {
    ...
    async getRealms(region, namespace, locale, status, timezone, orderbyField, pageNumber) {
        const oauthToken = await this.oauthClient.getToken();
        ...
        //const encodedCharacterName = encodeURIComponent(characterName);
          ...
          const realmListDocumentURL = `https://us.api.blizzard.com/data/wow/search/connected-realm?namespace=${namespace+region}&locale=${locale}&status.type=${status}&realms.timezone=${timezone}&orderby=${orderbyField}&_page=${pageNumber}`;
